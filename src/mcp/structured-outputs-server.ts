...
server.tool(
  ...
  {
    key: z.string().describe("The output key name (e.g., 'file_path', 'pr_url', 'status') - 'claude_' prefix will be added automatically"),
  ...
  async ({ key, value }) => {
    try {
      ...
      console.log(`Set GitHub Action output: ${prefixedKey}=${hasNewlines ? '[multiline]' : value}`);
...
server.tool(
  ...
  {
    outputs: z.record(z.string()).describe("Key-value pairs of outputs to set - 'claude_' prefix will be added automatically to all keys"),
  },
  async ({ outputs }) => {
    try {
      const githubOutput = process.env.GITHUB_OUTPUT;
      ...
      console.log(`Set ${keys.length} GitHub Action outputs:`, prefixedKeys.join(', '));
